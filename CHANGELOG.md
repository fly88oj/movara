...
- **Kimi Code adapter**: `~/.kimi-code` — workspaces.json (bucket keys + roots), session_index.jsonl (`sessionDir`/`workDir`), per-session state.json/wire.jsonl/tasks, and the `wd_<basename>_<sha256(root)[:12]>` buckets that name a sessions/ directory and file-history/ + workspace-trust/ files (bucket tokens ride the derived-token pairs; the creation-time `name` field follows the move when it still holds the old basename; the `.index-cache` scan cache is text-rewritten, the LMDB search-index and cache/logs/telemetry stay untouched).
